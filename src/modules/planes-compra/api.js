...
export const exportAcuerdosCom =(filters, callback) => {
    const queryParams = addFiltersQueryParams( '', filters )
    let url = `${process.env.REACT_APP_API_BASE_URL}ntr/acuerdo?formato=excel`
    console.log("url before get excel " , url)
    getHeaders().then( ( headers) => {
        var x=new XMLHttpRequest();
        x.open( "GET", `${process.env.REACT_APP_API_BASE_URL}ntr/acuerdo?fecha_desde=2021-01-01&formato=excel` , true);
        _.each(headers, (value, key) => {
            x.setRequestHeader( key, value );
        })
        x.responseType="blob";
...
export const updatePlansAcApi = (payload) => {
    return post(`/ntr/plan/estados`, { ...payload.change, planes: payload.plansIds } );
}
