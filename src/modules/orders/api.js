import {get, del, post, getHeaders} from '../../lib/restClient';
...
export const searchOrder = async ({
  ...
  queryParams = addFiltersQueryParams( queryParams, { codentidad_cbim, codpedido_origen, codcli_cbim, tipo, dates, codestado, idproducto })
  return get(`ntr/pedido?${queryParams}`);
};
...
export const realDeleteOrderById = (codpedido_origen, id_usuario) => post(`/procdef/borrar_pedido/launch`, [{name: 'codpedido_origen', value: codpedido_origen}, {name: 'usuario', value: id_usuario}, {name: 'origen', value: 'NTR-G' }]);
//export const changeOrderStatusById = (idOrder, status) => post(`ntr/pedido/${idOrder}/estado`, {codestado: status });
...
export const exportOrders =(filters, callback, error) => {
  const queryParams = addFiltersQueryParams( 'formato=excel', filters )
  getHeaders().then( ( headers) => {
    var x=new XMLHttpRequest();
    x.open( "GET", `${process.env.REACT_APP_API_BASE_URL}ntr/pedido?${queryParams}` , true);
