...
function* realDeleteOrderById({ payload }) {
  try {
    const response = yield call(api.realDeleteOrderById, payload.codpedido_origen, payload.id_usuario );
    const { data } = response;
    if (response.status === HttpStatus.UNAUTHORIZED) {
...
export function* watchrealDeleteOrderById() {
  yield takeLatest(REAL_DELETE_ORDER_BY_ID, realDeleteOrderById);
}
