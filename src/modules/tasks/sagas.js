...
function* fetchTaskList({ payload }) {
  try {
    ...
    let filterCounts = {
      type_order: _.where(defaultResponse.data, {formKey: 'validarPedido'}).length,
      type_approval: _.where(defaultResponse.data, {formKey: 'validarRegistro'}).length,
      type_forzar_mercancia: _.where(defaultResponse.data, {formKey: 'enviarMercanciaPendiente'}).length,
      type_entidad_nueva: _.where(defaultResponse.data, {formKey: 'validarEntidad'}).length,
      type_entidad_negativos: _.where(defaultResponse.data, {formKey: 'entidadPuntosNegativos'}).length,
      type_baja_voluntaria: _.where(defaultResponse.data, {formKey: 'bajaVoluntariaCliente'}).length,
