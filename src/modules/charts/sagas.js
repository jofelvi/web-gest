...
import {
 ...
 //fetchSalesByMonthSuccess,
  ...
  fetchEntitiesHourActivesSuccess,
    fetchStatsPedidosSuccess,
    fetchStatsPedidosPorGrupoSuccess,
    fetchStatsClientesSuccess,
...
import {
  ...
  FETCH_CLIENTS_DATA,
  FETCH_PENDING_TASKS,
    FETCH_STATS_CLIENTES,
  FETCH_STATS_PEDIDOS,
  FETCH_STATS_PEDIDOS_POR_GRUPO
...
function* fetchStatsPedidos({ payload }) {
  try {
    const response = yield call(api.getStatsPedidos, payload);
    yield put(fetchStatsPedidosSuccess( response ));
  } catch (e) {
    yield put(fetchStatsPedidosSuccess({ data: [] }  ) );
  }
}
export function* watchfetchStatsPedidos() {
  yield takeLatest(FETCH_STATS_PEDIDOS, fetchStatsPedidos);
}
function* fetchStatsPedidosPorGrupo({ payload }) {
  try {
    const response = yield call(api.getStatsPedidosPorGrupo, payload);

    yield put(fetchStatsPedidosPorGrupoSuccess( response ));
  } catch (e) {
    yield put(fetchStatsPedidosPorGrupoSuccess({ data: [] }  ) );
  }
}
export function* watchfetchStatsPedidosPorGrupo() {
  yield takeLatest(FETCH_STATS_PEDIDOS_POR_GRUPO, fetchStatsPedidosPorGrupo);
}
function* fetchStatsClientes({ payload }) {
  try {
    const response = yield call(api.getStatsClientes, payload);

    yield put(fetchStatsClientesSuccess( response ));
  } catch (e) {
    yield put(fetchStatsClientesSuccess({ data: [] }  ) );
  }
}
export function* watchfetchStatsClientes() {
  yield takeLatest(FETCH_STATS_CLIENTES, fetchStatsClientes);
}
