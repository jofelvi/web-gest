...
const TaskDetail = ({
  ...
  const processRegularizar = processId === 'vencimiento_plan' && taskName === 'enviarMercanciaPendiente';
  ...
  const processRegularizarPuntos = processId === 'incidencia_puntos' && taskName === 'entidadPuntosNegativos';

  const getProcessData = (taskName, processId) => {
    if(processRegularizar || processTramitarPedido || processRegistrarNuevaEntidad  || processRegularizarPuntos  || processRegistrarCliente){
  ...
  const dataForTableTab = (processRegularizar || processTramitarPedido || processRegistrarNuevaEntidad || processRegularizarPuntos || processRegistrarCliente)  && taskVariables && processD ? transformData(taskVariables, processD.processD) : '';
