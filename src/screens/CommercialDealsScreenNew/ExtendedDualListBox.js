...
class ExtendedDualListBox extends React.Component {
  ...
  allRightButtonClick() {
    ...
    const newSelectedKeys = _.union(
      selectedKeys,
      filteredUnselectedOptions.map((o) => o.value)
    );
  ...
  render() {
    ...
    const filteredOptions = _.filter(options, filter);
    const filteredUnselectedOptions = _.filter(filteredOptions, (option) => this.props.selectedKeys.indexOf(option.value) == -1);
    const selectedOptions = _.filter(options, (option) => this.props.selectedKeys.indexOf(option.value) > -1);
      ...
      <React.Fragment>
        <Row style={{ width: "100%" }}>
          ...
          <Col span={1} style={{ textAlign: "center" }}>
            <Button style={{ display: "inline-block", margin: "10px" }} onClick={this.allRightButtonClick}>
              <DoubleRightOutlined />
            </Button>
            <Button style={{ display: "inline-block", margin: "10px" }} onClick={this.rightButtonClick}>
              <RightOutlined />
            </Button>
            <Button style={{ display: "inline-block", margin: "10px" }} onClick={this.leftButtonClick}>
              <LeftOutlined />
            </Button>
            <Button style={{ display: "inline-block", margin: "10px" }} onClick={this.allLeftButtonClick}>
              <DoubleLeftOutlined />
            </Button>
          </Col>
          <Col span={11}>
