...
const PlanesCompraActions = (props) =>{
	...
	const confirmUpdatePlans= (keyName, valueName, key, value) =>{
		...
		const messageContent = selectedRowKeys.length > 0 ? `¿Desea cambiar \'${keyName}\' de los acuerdos comerciales ${acuerdos} a \'${valueName}\'?`
	...
	const updatePlans =( key, value )=> {
		...
		setLoading(false)
		let payload = {
			change: { [key]: value}, plansIds: selectedRowKeys
		}

		updatePlansAcApi( payload ).then(()=>{
				...
				}
				{
					selectedRowKeys.length >= 1 && (
						<React.Fragment>
							<Dropdown overlay={(
								<Menu>
									<Menu.Item key="1" onClick={() => { confirmUpdatePlans( 'Estado', 'Activo', 'idestado', '1') }}>
										Activo
									</Menu.Item>
									<Menu.Item key="2" onClick={() => { confirmUpdatePlans( 'Estado', 'Inactivo', 'idestado', '2') }}>
										Inactivo
									</Menu.Item>
								</Menu>
							)}>
