...
} from "../../../../modules/planes-compra/actions";
...
const defaultPlan = {
  ...
  productos: [],
  clientes: [{ idcliente: "" }],
  escalados: [{ udsminimas: 1, descuento: "", udsmaximas: "", txtdescuento: null }],
};
...
class PlanesCompraForm extends React.Component {
  ...
  validate(plan, successCallback, errorCallback) {
    ...
    const validations = [
      ...
      { field: "nombre", validator: (value) => value != "", message: "No se puede dejar en blanco" },
      {
        field: "fechainicio",
        validator: (value) =>
          editPlan && moment(editPlan.fechainicio).isSame(value, "day") ? true : moment(value).startOf("day") >= moment().startOf("day"),
        message: "No puede ser una fecha pasada.",
      },
      {
        field: "fechafin",
        validator: (value) =>
          editPlan && moment(editPlan.fechafin).isSame(value, "day") ? true : moment(value).startOf("day") >= moment().startOf("day"),
        message: "No puede ser una fecha pasada.",
      },
      {
        field: "fechafin",
  ...
  getError(field, spaced = false) {
    if (this.hasError(field)) {
      ...
      return (
        <Typography type="danger" style={{ color: "red" }}>
          {validationError}
        </Typography>
      );
    }
  ...
  filterSeleccionIndividual(item) {
    ...
    return (
      (filtro_categoria == "" || parseInt(item.idfamilia) == parseInt(filtro_categoria)) &&
      ...
      <ConfigProvider locale={locale}>
        <React.Fragment>
          {error && (
            <Typography type="danger" style={{ color: "red" }}>
              {" "}
              Se ha producido un error al guardar el plan, por favor, revisa los datos.
            </Typography>
          )}
          ...
          <div className="table-filters-indas" style={{ padding: "20px" }}>
                ...
                <div style={{ padding: "0px", paddingTop: "0", paddingRight: "20px" }}>
                  <OrderFilterEntity
                    ...
                    onChangeClient={(client) => {
                      this.setState({
                        rawPlan: { ...rawPlan, codcli_cbim: client.codcli_cbim },
                        plan: { ...plan, clientes: [{ idcliente: client.idcliente }] },
                      });
                    }}
                  />
                </div>
            ...
            <Row style={{ width: "100%", marginBottom: 0, paddingBottom: 0 }}>
              ...
              <Col span={18}>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    this.setState({ plan: { ...plan, descripcion: e.target.value } });
                  }}
                />
              </Col>
            </Row>
            ...
            <Row style={{ width: "100%", marginBottom: 0, paddingBottom: 0 }}>
              <Col span={6}>
                ...
                <Select
                  onChange={(value) => {
                    this.setState({ plan: { ...plan, idestado: value } });
                  }}
              ...
              <Col span={8}>
                <Switch
                  ...
                  disabled={isEditAndExpired}
                  onChange={(value) => {
                    this.setState({ plan: { ...plan, ind_renovar: value } });
                  }}
                />
              ...
              <Col span={8}>
                <Switch
                  ...
                  disabled={isEditAndExpired}
                  onChange={(value) => {
                    this.setState({ plan: { ...plan, ind_regularizar: value } });
                  }}
                />
            ...
            <Row style={{ width: "100%", marginBottom: 0, paddingBottom: 0 }}>
              ...
              >
                <TabPane tab="Selección por submarca" key="1">
                  <div style={{ top: "-80px", position: "relative" }}>{this.getError("submarcas")}</div>

                  {products && subBrands && (
                    <DiscriminatorListBox
                      disabled={isEditAndExpired}
                      options={_.sortBy(products, (product) => product.nombre).map((product) => ({
                        label: product.nombre,
                        value: product.idproducto,
                        discriminator: parseInt(product.idsubmarca),
                      }))}
                      discriminator_options={_.sortBy(subBrands, (subBrand) => subBrand.nombre).map((subBrand) => ({
                      ...
                      preset={{
                        ...
                        options: submarcaCollections.map((submarcaCollection) => ({
                          label: submarcaCollection.nombre,
                          value: submarcaCollection.nombre,
                          options: submarcaCollection.submarcas,
                        })),
                ...
                <TabPane tab="Selección Individual" key="2">
                  <div style={{ top: "-80px", position: "relative" }}>{this.getError("productos")}</div>
          ...
          {error && (
            <Typography type="danger" style={{ color: "red", marginTop: "10px" }}>
              {" "}
              Se ha producido un error al guardar el plan, por favor, revisa los datos.
            </Typography>
          )}
          {isEditAndExpired && (
            <Typography type="danger" style={{ marginTop: "10px" }}>
              {" "}
              El plan no se puede modificar porque ha expirado.
            </Typography>
          )}
